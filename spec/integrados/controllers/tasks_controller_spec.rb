...
RSpec.describe TasksController, type: :controller do
    ...
    describe 'POST#create' do
        ...
        context 'erro' do
            ...
            it 'não deve criar um registro na tabela de tarefas' do
                expect {post :create, params: @params}.to change(Task, :count).by(0)
            end
    ...
    describe 'PUT#change_status' do
        context 'sucesso' do
            ...
            it 'deve retornar mensagem de sucesso' do
                put :change_status, params: @params
                expect(response.request.flash[:notice]).to eq("Task was successfully update.")
            end
        end

        context 'erro' do
            before do
                @params = {"project_id"=> project.id, "task_id"=> ""}
            end
