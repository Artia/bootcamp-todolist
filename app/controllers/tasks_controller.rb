class TasksController < ApplicationController
  ...
  def create
    ...
    @task = task_service.create(task_params, @project.id)
  ...
  def update
    respond_to do |format|
      if task_service.update(task_params, params[:id], @project.id)
  ...
  def destroy
    ...
    task_service.destroy(params[:id], @project.id)
    # @task.destroy
  ...
  def change_status
    begin
      ...
      respond_to do |format|
        format.html{redirect_to project_tasks_url(@project), notice: 'Task was successfully update.'}
        format.json { head :no_content }
      end
