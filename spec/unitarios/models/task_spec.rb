...
RSpec.describe Task, type: :model do
    ...
    describe 'validação' do
        ...
        describe 'date' do
            ...
            it 'não deve aceitar data de ínicio maoir que data de fim' do
                #setup
    ...
    describe 'validate_situation' do
        it 'state late' do
            task.stub(date_end: Time.now - 1)
            expect(task.validate_situation).to eq('Late')
        end
        
        it 'state on time' do
            task.stub(date_end: Time.now + 1)
            expect(task.validate_situation).to eq('On time')
        end

        it 'state -' do
            task.stub(state: true)
            expect(task.validate_situation).to eq('-')
        end
    end
